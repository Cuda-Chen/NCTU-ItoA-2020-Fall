// https://github.com/algo-seacow/NCTU-ItoA-2020-Fall/tree/master/1131
// multiple times single-source shortest path
// I have no idea why you guys can come up such ideas.
#include <cstdio>
#include <vector>
#include <algorithm>
#include <queue>
...
int main() {
    ...
    int ans = min(distance_from_u[v] + distance_from_v[w] + distance_from_w[u],
                  distance_from_u[w] + distance_from_v[u] + distance_from_w[v]);
    if(ans >= INF) ans = -1;
    printf("%d\n", ans);
