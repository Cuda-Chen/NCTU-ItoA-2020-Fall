// https://github.com/algo-seacow/NCTU-ItoA-2020-Fall/blob/master/1117/submissions/accepted/309552025_radix_sort.cpp
// radix sort
#include <cstdio>
#include <vector>
#include <algorithm>
...
int main() {
    ...
    vector<int> dup_x(n), dup_y(n), dup_z(n), dup_xy(n), dup_yz(n), dup_xz(n), dup_xyz(n);
    ...
    // sorted by z after sorted by x and y
    {
        ...
        for(int i = 0; i < n; ) {
            int st = i, ed = i + 1;
            while(ed < n && copy_points[st].x == copy_points[ed].x && copy_points[st].y == copy_points[ed].y && copy_points[st].z == copy_points[ed].z) ed++;
            while(i < ed) {
    ...
    // sort by x after sorted by, x, y, z
    {
        vector<int> cnt(n), prefix_sum(n);
        for(int i = 0; i < n; i++) cnt[copy_points[i].x]++;
        prefix_sum[0] = cnt[0];
